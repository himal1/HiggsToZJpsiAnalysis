...
void PATEventTree::beginJob() {
  ...
  fTree->Branch("HLT_Muon_VertexmumuJpsi",       fHLT_Muon_VertexmumuJpsi,          "HLT_Muon_VertexmumuJpsi[HLTN]/F");
   ...
   fTree->Branch("HLT_MuonUpsilon_L3Matching_Phi", fHLT_MuonUpsilon_L3Matching_Phi,          "HLT_MuonUpsilon_L3Matching_Phi[5]/F");
   ...
   fTree->Branch("HLT_MuonZ_VtxMatching_Pt",  fHLT_MuonZ_VtxMatching_Pt,          "HLT_MuonZ_VtxMatchingZ_Pt[5]/F");
   fTree->Branch("HLT_MuonZ_VtxMatching_Eta", fHLT_MuonZ_VtxMatching_Eta,          "HLT_MuonZ_VtxMatchingZ_Eta[5]/F");
   fTree->Branch("HLT_MuonZ_VtxMatching_Phi", fHLT_MuonZ_VtxMatching_Phi,          "HLT_MuonZ_VtxMatchingZ_Phi[5]/F");
   //end
   fTree->Branch("HLT_Muon_VertexmumuJpsi",       fHLT_Muon_VertexmumuJpsi,          "HLT_Muon_VertexmumuJpsi[HLTN]/F");
   fTree->Branch("HLT_Muon_TripleMuL3",         fHLT_Muon_TripleMuL3,          "HLT_Muon_TripleMuL3[HLTN]/F");
   //end                                                                                                                                                      
...
/*  
  ...
  std::string filter3 = "hltVertexmumuFilterJpsiMuon";// The last trigger selected..
  std::string filter2 = "hltTripleMuL3PreFiltered0"; //General Trigger path which first trigger our Trigger
*/
  // std::string path = "HLT_Dimuon0_Jpsi_Muon_v4";
  ///std::string path = "HLT_Dimuon0_Jpsi3p5_Muon2_v4";
  ...
  if ( fHLT_Skim && !( fHLTP_Trimuon5_3p5_2_Upsilon_Muon[1] || fHLTP_Dimuon0_Jpsi3p5_Muon2[1] || fHLTP_IsoMu20[1] || fHLTP_Mu50[1])) return; 
 // if ( fHLT_Skim && !(fHLTP_Dimuon0_Upsilon_Muon[1] || fHLTP_Dimuon0_Jpsi_Muon[1] || fHLTP_QuadMuon0_Dimuon0_Jpsi[1] || fHLTP_QuadMuon0_Dimuon0_Upsilon[1]) ) return; // only ntuplize events passing trigger if skimming enabled
  ...
  if (fIsMC) {
    ...
    //stefan himal MC truth matching
    edm::Handle<reco::GenParticleCollection> genHandle;
    iEvent.getByLabel(fGenCollectionLabel, genHandle);
    int GEN=-1;
    for (auto&& genPart : *(genHandle.product())){

      //if (genPart.status==1 && std::abs(genPart.pdgId())==13){//previous                                                                                                          
      if(std::abs(genPart.pdgId()) == 23) {
	//cout << "Have a Z" << endl;

	//cout << "Daughters : " << genPart.numberOfDaughters() << endl;
	if(genPart.numberOfDaughters()==2){
	  GEN++;
	  //cout << "1. daughter: " << genPart.daughter(0)->pdgId() << endl;
	  //cout << "2. daughter: " << genPart.daughter(1)->pdgId() << endl;


	  //std::cout<<"Pt of the generated muon 1-->"<<genPart.daughter(0)->pt()
	  //<<" Eta " << genPart.daughter(0)->eta() 
	  //	   <<" phi "<< genPart.daughter(0)->phi() <<std::endl;

	  //std::cout<<"Pt of the generated muon 2-->"<<genPart.daughter(1)->pt()
	  //	   <<" Eta " << genPart.daughter(1)->eta() 
	  //	   <<" phi "<< genPart.daughter(1)->phi() <<std::endl;
	  
	  fGenMuonPt[0]=genPart.daughter(0)->pt();
	  fGenMuonEta[0]= genPart.daughter(0)->eta();
	  fGenMuonPhi[0]=genPart.daughter(0)->phi();
	  fGenMuonPt[1]=genPart.daughter(1)->pt();
	  fGenMuonEta[1]= genPart.daughter(1)->eta();
	  fGenMuonPhi[1]=genPart.daughter(1)->phi();
	} 

      } // end Z
      if(std::abs(genPart.pdgId()) == 443) {
	//cout << "Have a Ups" << endl;
	...
	if(genPart.numberOfDaughters()==2){
	  ...
	  fGenMuonPt[2]=genPart.daughter(0)->pt();
	  fGenMuonEta[2]= genPart.daughter(0)->eta();
	  fGenMuonPhi[2]=genPart.daughter(0)->phi();
	  fGenMuonPt[3]=genPart.daughter(1)->pt();
	  fGenMuonEta[3]= genPart.daughter(1)->eta();
	  fGenMuonPhi[3]=genPart.daughter(1)->phi();
	}
  ...
  fillTopology();
  //open both for general purpose
  //only closed for GEN level Plot
...
void PATEventTree::init() {
  ...
  // Only for MC
  //HIMAL ADDITION FOR gen
  if (fIsMC) {
    for (int hii=0; hii<4; hii++){
      fGenMuonPt[hii]=-9999;
      fGenMuonEta[hii]=-9999;
      fGenMuonPhi[hii]=-9999;
    }
  }
  //end 
  if (fIsMC) {
  ...
  fHLTP_DoubleMu3_PS = fHLTP_DoubleMu6_PS = fHLTP_DoubleMu7_PS = fHLTP_Dimuon0_Upsilon_Muon_PS = fHLTP_Dimuon0_Jpsi3p5_Muon2_PS = fHLTP_Dimuon0_Jpsi_PS = fHLTP_Dimuon10_Jpsi_Barrel_PS = fHLTP_TripleMu5_PS = fHLTP_QuadMuon0_Dimuon0_Jpsi_PS = fHLTP_QuadMuon0_Dimuon0_Upsilon_PS = fHLTP_Trimuon5_3p5_2_Upsilon_Muon_PS = fHLTP_Trimuon2_Upsilon5_Muon_PS = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon_PS =fHLTP_IsoMu20_PS=fHLTP_IsoMu27_PS = fHLTP_Mu50_PS= 0;

  for (int i = 0; i < 3; i++) {
    fHLTP_DoubleMu3[i] = fHLTP_DoubleMu6[i] = fHLTP_DoubleMu7[i] = fHLTP_Dimuon0_Upsilon_Muon[i] = fHLTP_Dimuon0_Jpsi3p5_Muon2[i] = fHLTP_Dimuon0_Jpsi[i] = fHLTP_Dimuon10_Jpsi_Barrel[i] = fHLTP_TripleMu5[i] = fHLTP_QuadMuon0_Dimuon0_Jpsi[i] = fHLTP_QuadMuon0_Dimuon0_Upsilon[i] = fHLTP_Trimuon5_3p5_2_Upsilon_Muon[i] = fHLTP_Trimuon2_Upsilon5_Muon[i] = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon[i] = fHLTP_IsoMu20[i]= fHLTP_IsoMu27[i] = fHLTP_Mu50[i] = false;
  }
  for (int i=0; i<11; i++){
    fHLTP_Trimuon5_3p5_2_Upsilon_Muon_Filters[i]=false;
  }
  for (int i = 0; i < 5; i++) {
    fHLTP_DoubleMu3_Filters[i] = fHLTP_DoubleMu6_Filters[i] = fHLTP_DoubleMu7_Filters[i] = fHLTP_TripleMu5_Filters[i] = fHLTP_Dimuon0_Jpsi_Filters[i] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[i] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[i] = fHLTP_IsoMu20_Filters[i]=fHLTP_IsoMu27_Filters[i] = fHLTP_Mu50_Filters[i] = false;
  }
  fHLTP_IsoMu20_Filters[5]=fHLTP_IsoMu20_Filters[6]=fHLTP_IsoMu27_Filters[5]=fHLTP_IsoMu27_Filters[6]=fHLTP_Mu50_Filters[5]=fHLTP_Dimuon0_Jpsi_Filters[5] = fHLTP_Dimuon0_Jpsi_Filters[6] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[5] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[6] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[7] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[5] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[6] = false;
...
void PATEventTree::fillHLTPath(const unsigned int psSet, const edm::Handle<edm::TriggerResults>& hHLTresults, const trigger::TriggerEvent& triggerEvent) {
  ...
  for (unsigned int itrig = 0; itrig < validTriggerNames.size(); itrig++) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_Dimuon0_Jpsi3p5_Muon2_v")!=string::npos) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_Trimuon5_3p5_2_Upsilon_Muon_v")!=string::npos) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_IsoMu20_v")!=string::npos) {
      fHLTP_IsoMu20[0] = wasrun;
      fHLTP_IsoMu20[1] = accept;
      fHLTP_IsoMu20[2] = error;
      fHLTP_IsoMu20_PS = prescale;
    } else if (validTriggerNames[itrig].find("HLT_IsoMu27_v")!=string::npos) {
      fHLTP_IsoMu27[0] = wasrun;
      fHLTP_IsoMu27[1] = accept;
      fHLTP_IsoMu27[2] = error;
      fHLTP_IsoMu27_PS = prescale;      
    }  else if (validTriggerNames[itrig].find("HLT_Mu50_v")!=string::npos) {
      fHLTP_Mu50[0] = wasrun;
      fHLTP_Mu50[1] = accept;
      fHLTP_Mu50[2] = error;
      fHLTP_Mu50_PS = prescale;
      
    } else if (validTriggerNames[itrig].find("HLT_Trimuon2_Upsilon5_Muon_v")!=string::npos) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_TrimuonOpen_5_3p5_2_Upsilon_Muon_v")!=string::npos) {
  ...
  for(uint filterIndex=0;filterIndex<triggerEvent.sizeFilters();++filterIndex){  //loop over all trigger filters in event (i.e. filters passed)
    ...
    if(label.find( "hltL1sL1DoubleMu0" ) != string::npos ) {
      ...
      //cout << "Trigger filter JPsifound in path method"<<endl;
      ...
      //addition of filter for Z trigger
    } else if(label.find( "hltL1sSingleMu18" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[0]=true;
    } else if(label.find( "hltPreIsoMu20" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[1]=true;
    } else if(label.find( "hltL1fL1sMu18L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[2]=true;
    } else if(label.find( "hltL2fL1sMu18L1f0L2Filtered10Q" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[3]=true;
    } else if(label.find( "hltL1fForIterL3L1fL1sMu18L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[4]=true;
    } else if(label.find( "hltL3fL1sMu18L1f0L2f10QL3Filtered20Q" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[5]=true;
    } else if(label.find( "hltL3crIsoL1sMu18L1f0L2f10QL3f20QL3trkIsoFiltered0p07" ) != string::npos ) {
      fHLTP_IsoMu20_Filters[6]=true;
    } else if(label.find( "hltL1sSingleMu22or25" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[0]=true;
    } else if(label.find( "hltPreIsoMu27" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[1]=true;
    } else if(label.find( "hltL1fL1sMu22or25L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[2]=true;
    } else if(label.find( "hltL2fL1sMu22or25L1f0L2Filtered10Q" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[3]=true;
    } else if(label.find( "hltL1fForIterL3L1fL1sMu22or25L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[4]=true;
    } else if(label.find( "hltL3fL1sMu22Or25L1f0L2f10QL3Filtered27Q" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[5]=true;
    } else if(label.find( "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[6]=true;
    } else if(label.find( "hltL1sSingleMu22or25" ) != string::npos ) {
      fHLTP_Mu50_Filters[0]=true;
    } else if(label.find( "hltPreMu50" ) != string::npos ) {
      fHLTP_Mu50_Filters[1]=true;
    } else if(label.find( "hltL1fL1sMu22or25L1Filtered0" ) != string::npos ) {
      fHLTP_Mu50_Filters[2]=true;
    } else if(label.find( "hltL2fL1sMu22or25L1f0L2Filtered10Q" ) != string::npos ) {
      fHLTP_Mu50_Filters[3]=true;
    } else if(label.find( "hltL1fForIterL3L1fL1sMu22or25L1Filtered0" ) != string::npos ) {
      fHLTP_Mu50_Filters[4]=true;
    } else if(label.find( "hltL3fL1sMu22Or25L1f0L2f10QL3Filtered50Q" ) != string::npos ) {
      fHLTP_Mu50_Filters[5]=true;
      //end of Z triggers
...
void PATEventTree::fillHLT(const pat::TriggerObjectStandAloneCollection& HLTObjects) {
  ...
  //initialiaziation
  int pass1,pass2,passUps1,passUps2,passZ;
  pass1=pass2=passUps1=passUps2=passZ=0;
  int imuon(0);
  
  //std::string filter1 = "hltJpsiMuonL3Filtered"; //filter1 and filter2 or filter 3 
  std::string filter3 = "hltVertexmumuFilterJpsiMuon3p5";// The last trigger selected..
  std::string filter2 = "hltTripleMuL3PreFiltered222"; //General Trigger path which first trigger our Trigger
  std::string filterZ1 = "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07"; //L3 Trigger Path for Z trigger Iso27
  std::string filterZ = "hltL3crIsoL1sMu18L1f0L2f10QL3f20QL3trkIsoFiltered0p07"; //L3 Trigger Path for Z trigger Iso20
  ...
  for ( int ih = 0; ih < (int) HLTObjects.size(); ih++) {
    ...
    //HImal Addition for Jpsi trigger    
	...
	++pass1;
	if (pass1==5) {
	  cout<<"maximum limit on th L3 pass muon "<<endl;
	  continue;
	}
    ...
    // endloop find muons etc. for trigger
    ...
    if( HLTObjects[fHLTN].hasPathName("HLT_Dimuon0_Jpsi3p5_Muon2_v5", false, false) ) fHLT_Dimuon0_Jpsi3p5_Muon2[fHLTN][0] = true;
  ...
  //if (pass2>0){
    //cout << "The trigger found muons : " << pass2 << endl;
    //cout<< "HLT found muon 1 Pt : "<<fHLT_MuonJpsi_VtxMatching_Pt[0]<<endl;
    //cout<< "HLT found muon 2 Pt : "<<fHLT_MuonJpsi_VtxMatching_Pt[1]<<endl;
    //cout<< "HLT found muon 3 Pt : "<<fHLT_MuonJpsi_VtxMatching_Pt[2]<<endl;
    //cout<< "HLT found muon 4 Pt : "<<fHLT_MuonJpsi_VtxMatching_Pt[3]<<endl;
    //cout<< "HLT found muon 5 Pt : "<<fHLT_MuonJpsi_VtxMatching_Pt[4]<<endl;
    //}
  
  //cout<<"Number of matched muon in L3 and Vertexing in Upsilon are -->"<<passUps1<< " and "<<passUps2<<endl; 
  //cout<<"Number of matched muon in L3 and Vertexing in JPsi are -->"<<pass1<< " and "<<pass2<<endl;
  //cout<<"Number of matched muon in L3 in Z are -->"<<passZ<<endl; 
}
...
void PATEventTree::fillJPsiMuMuCand(const reco::CompositeCandidateCollection& JPsiCands, const reco::MuonRefVector& goodMuons, std::vector<TransientTrack>& t_tks) {
  ...
  for(unsigned int ijp = 0; ijp < JPsiCands.size(); ijp++) {
    ...
    if( fJPsiMuCategory[fJPsiN][0]>0 && fJPsiMuCategory[fJPsiN][1]>0 && fJPsiMass[0]>8.0 && fJPsiMass[0]<11.0 ) {//changed for Upsilon
...
void PATEventTree::makeEtabCand(std::vector<TransientTrack>& t_tks) {
  ...
  for(int ijp1 = 0; ijp1 < fJPsiMuMuN-1; ijp1++) {
    ...
    for(int ijp2 = ijp1+1; ijp2 < fJPsiMuMuN; ijp2++) {
      ...
      //     if ((fJPsiMass[ijp1] < 2.80 || fJPsiMass[ijp1] > 3.4 || fJPsiMass[ijp2] < 2.80 || fJPsiMass[ijp2] > 3.4)&&( fJPsiMass[ijp1] < 8.0 || fJPsiMass[ijp1] > 12.0 || fJPsiMass[ijp2] < 8.0 || fJPsiMass[ijp2] > 12.0)) continue; //previous
      //----------Himal addition of cut-------------------
      if (((fJPsiMass[ijp1] < 2.80 || fJPsiMass[ijp1] > 3.4)&&(fJPsiMass[ijp1] < 75 || fJPsiMass[ijp1] > 110)&&(fJPsiMass[ijp1] < 8.5 || fJPsiMass[ijp1] > 12.0)) || ((fJPsiMass[ijp2] < 2.80 || fJPsiMass[ijp2] > 3.4)&&(fJPsiMass[ijp2] < 75 || fJPsiMass[ijp2] > 110) && (fJPsiMass[ijp2] < 8.5 || fJPsiMass[ijp2] > 12.0))) continue; //Himal change for Z J Y  mass cut
      //himal addition of Cut
      //-------------JpsiVtx  cut-------------
      //if (TMath::Prob(fJPsiChi2[ijp1],fJPsiNdof[ijp1])<0.01) continue;
      //if (TMath::Prob(fJPsiChi2[ijp2],fJPsiNdof[ijp2])<0.01) continue;
      //------------muon pt cut...................
      ...
      //cout <<"muon pt 1--->"<<fPcPt[fJPsiMuI[ijp1][0]]<<endl;
      //cout <<"muon pt 2--->"<<fPcPt[fJPsiMuI[ijp1][1]]<<endl;
      //cout <<"muon pt 3--->"<<fPcPt[fJPsiMuI[ijp2][0]]<<endl;
      //cout <<"muon pt 4--->"<<fPcPt[fJPsiMuI[ijp2][1]]<<endl;
      ...
      //if (fEtabProb[fEtabN]<0.01) continue;
